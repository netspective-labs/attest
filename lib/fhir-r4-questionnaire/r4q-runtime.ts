...
function preferredNameFromItem(item: FhirQItem): string | undefined {
  const codes = Array.isArray((item as Any).code) ? (item as Any).code : [];
  const first = codes.find((x: Any) =>
    x && typeof x.code === "string" && x.code.trim()
  );
  return first?.code?.trim();
}

function sanitizeIdentifier(s: string): string {
  // collapse non-alphanumerics to spaces, drop stray chars, trim
  const cleaned = s.replace(/[^A-Za-z0-9]+/g, " ").trim();
  // turn to camelCase (assuming you have toCamelCase helper already)
...
export function flattenItems(
  ...
  for (const it of items) {
    ...
    const rawName = preferredNameFromItem(it) ?? it.text ?? it.linkId;
    ...
    const tsType = repeats ? (union ? `(${union})[]` : `${baseTs}[]`) : baseTs;
...
export function findQrAnswersByLinkId(
  qr: Any,
  linkId: string,
): unknown[] | undefined {
  if (!qr) return undefined;
