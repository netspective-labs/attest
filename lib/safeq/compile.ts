...
function leafSchemaForItem(
  ...
  switch (t) {
    ...
    case "open-choice": {
      ...
      if (mode === "coding") {
        return allowFree ? z.union([codingSchema, allowFree]) : codingSchema;
      }
      ...
      if (fromOptions) {
        return allowFree ? z.union([fromOptions, allowFree]) : fromOptions;
      }
      return allowFree ? z.union([codingSchema, allowFree]) : codingSchema;
    }
    default:
        ...
        if (it.type === "group") {
          ...
          const projectChild = (node: Any) => {
            ...
            for (const child of childItems) {
              ...
              if (child.type === "group") {
                const groupNodes = scopedIndex[child.linkId ?? ""] ?? [];
                if (!groupNodes.length) continue;
                const groupVals = groupNodes.map(projectChild);
                obj[ck] = child.repeats ? groupVals : groupVals[0];
              } else {
                obj[ck] = readLeaf(
                  child,
