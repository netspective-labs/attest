...
export const questionnaireEnableOperatorSchema = z.enum([
    ...
    "<",
    "<=",
]).transform((v) => v.trim() as typeof v); // keep tight but resilient to accidental spaces

// value[x] unions — OPEN & STRICT
...
const questionnaireItemEnableWhenOpenSchema = z.union([
    ...
    }).and(oneOfAnswerValueOpenSchema),
]);
...
const questionnaireItemEnableWhenStrictSchema = z.union([
    ...
    }).and(oneOfAnswerValueStrictSchema),
]);
